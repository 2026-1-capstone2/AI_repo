...
import torch
from app.vlm.llava.model import *
from app.vlm.llava.constants import DEFAULT_IMAGE_PATCH_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN
...
def load_pretrained_model(model_path, model_base, model_name, load_8bit=False, load_4bit=False, device_map="auto", torch_dtype="float16",attn_implementation="flash_attention_2", customized_config=None, overwrite_config=None, **kwargs):
    ...
    if load_8bit:
        ...
        if "max_memory" not in kwargs and torch.cuda.is_available():
            total_vram_gb = torch.cuda.get_device_properties(0).total_memory / (1024 ** 3)
            # 활성화/KV cache용으로 ~1GB 남기고 나머지를 weight 예산으로
            llm_weight_budget = max(1.5, total_vram_gb - 1.0)
            kwargs["max_memory"] = {0: f"{llm_weight_budget:.1f}GiB", "cpu": "32GiB"}
    ...
    if "llava" in model_name.lower() or is_multimodal:
        ...
        if "lora" in model_name.lower() and model_base is not None:
            ...
            if "mixtral" in model_name.lower():
                from app.vlm.llava.model.language_model.llava_mixtral import LlavaMixtralConfig
                ...
                from app.vlm.llava.model.language_model.llava_mistral import LlavaMistralConfig
                ...
                from app.vlm.llava.model.language_model.llava_gemma import LlavaGemmaConfig
                ...
                from app.vlm.llava.model.language_model.llava_qwen import LlavaQwenConfig
                ...
                _low_cpu = True if (load_4bit or load_8bit) else False
                if overwrite_config is not None:
                    overwrite_config.update(additional_config)
                    rank0_print(f"Overwriting config with {overwrite_config}")
                    for k, v in overwrite_config.items():
                        setattr(lora_cfg_pretrained, k, v)
                    model = LlavaQwenForCausalLM.from_pretrained(model_base, low_cpu_mem_usage=_low_cpu, attn_implementation=attn_implementation, config=lora_cfg_pretrained, **kwargs)
                else:
                ...
                from app.vlm.llava.model.language_model.llava_llama import LlavaConfig
            ...
            if load_4bit or load_8bit:
                for name, param in non_lora_trainables.items():
                    ...
                    setattr(module, parts[-1], torch.nn.Parameter(param.to(device=model.device, dtype=torch.float16)))
            else:
                model.load_state_dict(non_lora_trainables, strict=False)

            from peft import PeftModel

            # PEFT가 LoRA 추가 후 device_map을 자체 재계산하는데, 인자 없으면 max_memory=None으로
            # get_balanced_memory를 호출 → 저VRAM 환경에서 GPU 예산을 0으로 잡아 전부 CPU 배치 →
            # dispatch_model이 main_device 못 찾아 IndexError. max_memory를 명시 전달해 회피.
            peft_load_kwargs = {}
            if "max_memory" in kwargs:
                peft_load_kwargs["max_memory"] = kwargs["max_memory"]

            rank0_print("Loading LoRA weights...")
            model = PeftModel.from_pretrained(model, model_path, **peft_load_kwargs)
            if load_4bit or load_8bit:
                rank0_print("Skipping LoRA merge (quantized model — keeping adapter)...")
            else:
            ...
            if "mixtral" in model_name.lower():
                ...
                if "moe" in model_name.lower() or "A14B" in model_name.lower():
                    from app.vlm.llava.model.language_model.llava_qwen_moe import LlavaQwenMoeConfig
                    if overwrite_config is not None:
